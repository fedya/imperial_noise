...
remove_noise() {
	...
	#$SOX $input_file noise.wav trim 1 1.4
	...
	$SOX "$input_file"_cleaned.wav -r 8000 -c2  lowpass 4000 rate
	echo "[wav to mp3 convertation, need installed lame codec]"
	ffmpeg -i "$input_file"_cleaned.wav -vn -ar 44100 -ac 2 -ab 192 -f mp3 "$input_file"_cleaned.mp3
}
